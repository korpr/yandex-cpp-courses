...
bool IsMale(const Person& p){
    return p.gender == Gender::MALE;
}

bool IsEmpoyed(const Person& p){
    return p.is_employed;
}
void PrintStats(vector<Person> persons) {
    auto females_end = partition(begin(persons), end(persons), IsMale);
    ...
    auto employed_females_end = partition(begin(persons), females_end, IsEmpoyed);
    ...
    auto employed_males_end = partition(females_end, end(persons), IsEmpoyed);
