...
vector<Token> Tokenize(istream& cl) {
  ...
  while (cl >> c) {
    if (isdigit(c)) {
      string date(1, c);// create  sting with size 1 and contain c
      for (int i = 0; i < 3; ++i) { //read yy-mm-dd
      ...
      tokens.push_back({date, TokenType::DATE});//create token
    } else if (c == '"') {
      string event;
      getline(cl, event, '"'); // copy event  into event))) until next "
